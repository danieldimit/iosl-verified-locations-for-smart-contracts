...
contract Test {
      bytes32 position = "u33hac";
      bytes8 geofence_prefix ="u33";
      bytes4[] geofence_suffix;
      bool leftGeofence = false;
      
      function CarDetails(){
          geofence_suffix.push("h");
          geofence_suffix.push("k");
          geofence_suffix.push("s");
          geofence_suffix.push("u");
          geofence_suffix.push("7");
      }
      
      /*
    function getGeofenceSuffix() returns(bytes4[]){
        return geofence_suffix;
    }
    function getPosition() returns(bytes32){
        return position;
    }
    function geofenceSuffixLength() returns(uint){
        return geofence_suffix[0].length;
    }
    
    
    function checking() returns(bool){
             if(leftGeofence==false){
                 leftGeofence=true;
              for(uint j; j<geofence_suffix.length;j++){
                  bytes4 temp = geofence_suffix[j];
     ...
     function checkPositionInGeofenceGeohash() returns(bool) {
          for(uint i;i<3;i++){
              if(geofence_prefix[i]!=position[i]){
                  leftGeofence = true;
                  break;
